...
- **Encryption at rest**: Secrets stored encrypted in `~/.pdk/secrets.json`
...
1. **Never commit secrets**: Add `~/.pdk/secrets.json` to `.gitignore`
...
```bash
ls -la ~/.pdk/secrets.json
# Should be: -rw------- (0600)
chmod 600 ~/.pdk/secrets.json
```
